...
async function callGeminiOnce(model, inputParts, aspectRatio, imageSize) {
    const t0 = Date.now();
...
const FALLBACK_NOTICE = `\u26A0\uFE0F Generated with ${MODEL_FALLBACK} (Gemini 2.5 Flash Image) — the free-tier-eligible fallback. ` +
...
function fallbackNoticeIfNeeded(modelUsed) {
    ...
    try {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            if (result.status === "fulfilled") {
                anySucceeded = true;
                if (result.value.modelUsed !== MODEL_PRIMARY)
                    anyFallback = true;
                content.push({
                    type: "text",
    ...
    try {
        ...
        return {
            content: [
                ...
                { type: "text", text: `Saved as ${filename} — full-res at http://localhost:${viewerPort}${fallbackNoticeIfNeeded(modelUsed)}` },
            ],
        };
    }
    catch (err) {
...
server.tool("edit_image", `Edit an existing image using Google's Nanobanana2 (Gemini 3.1 Flash Image). Provide the filename of an image in ${SAVE_DIR} (use list_images to see available files, or save_image to import one first). The MCP reads the file directly — do NOT pass base64 image data.`, {
    ...
    try {
        ...
        const { mcpBase64, mcpMimeType, text, filename: outFilename, modelUsed } = await editAndStore(styledPrompt, srcBase64, srcMime, resolvedAR, image_size);
        log(`edit_image complete in ${((Date.now() - t0) / 1000).toFixed(1)}s`);
        return {
            content: [
                ...(text ? [{ type: "text", text }] : []),
                { type: "image", data: mcpBase64, mimeType: mcpMimeType },
                { type: "text", text: `Saved as ${outFilename} — full-res at http://localhost:${viewerPort}${fallbackNoticeIfNeeded(modelUsed)}` },
            ],
        };
    }
    catch (err) {
...
server.tool("interactive_fix", `Opens an image in a browser-based crop tool where the user can draw a rectangle around the region to fix, add notes/instructions, and submit. The tool waits for the user's selection, then sends the cropped region to Gemini for repair and composites it back into the original image. Best for precise, user-guided fixes.`, {
    ...
    try {
        ...
        const geminiResults = await Promise.allSettled(Array.from({ length: shots }, (_, i) => callGemini([
            { text: prompt },
            { inlineData: { mimeType: sendMime, data: sendBuf.toString("base64") } },
        ], snapped.aspectLabel, image_size).then(async ({ imageBase64, modelUsed }) => {
        ...
        return {
            content: [
                ...
                { type: "image", data: mcpBase64, mimeType: mcpMimeType },
                { type: "text", text: `Saved as ${chosenFilename} — full-res at http://localhost:${viewerPort}${fallbackNoticeIfNeeded(chosenModel)}` },
            ],
        };
    }
    catch (err) {
