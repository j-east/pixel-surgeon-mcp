...
function loadKeysFromClaudeConfig(): { google: string; openai: string; xai: string } {
  try {
...
type ProviderName = "gemini" | "openai" | "xai";
...
const RESPIN_ASPECTS = ["1:1", "16:9", "9:16", "3:4", "4:3", "2:3", "3:2", "4:5", "5:4"] as const;
...
async function writeSidecar(img: StoredImage): Promise<void> {
      ...
      if (url.pathname === "/history" && req.method === "GET") {
        (async () => {
          try {
            ...
            const readMeta = async (f: string): Promise<Meta | null> => {
              try {
            ...
            if (q) {
              ...
              for (const f of jsonFiles) {
                ...
                if (meta && (meta.prompt ?? "").toLowerCase().includes(q)) all.push(meta);
              }
              total = all.length;
              ...
              items = loaded.filter((m): m is Meta => m !== null);
            }
...
function viewerHtml(): string {
  ...
  const latestImage = items.find(i => i.type === "image")?.data as StoredImage | undefined;
...
function grokAspect(aspectRatio: string): string {
  return GROK_ASPECT_MAP[aspectRatio] ?? "1:1";
}

class GrokProvider implements ImageProvider {
  name = "xai";
...
const STYLE_PRESETS: Record<string, { description: string; promptPrefix: string; defaultAspectRatio?: string }> = {
  "neo-brutalist": {
...
function resolveAspectRatio(explicit: string, style?: string): string {
  if (explicit !== "1:1") return explicit; // user explicitly chose something
  if (style && STYLE_PRESETS[style]?.defaultAspectRatio) return STYLE_PRESETS[style].defaultAspectRatio!;
  return explicit;
}
  ...
  async ({ filename, image_size, model }) => {
    ...
    try {
      ...
      if (succeeded.length === 0) {
        const firstErr = (geminiResults[0] as PromiseRejectedResult).reason?.message || "Unknown error";
...
server.tool(
  "remove_background",
  `Remove a background color from an image and make it transparent. Outputs a PNG with alpha channel. The image must already exist in ${SAVE_DIR} (use save_image to import first).`,
  {
    filename: z.string().describe(`Filename of the source image in ${SAVE_DIR}`),
    color: z
      .string()
...
function traceAsync(
  ...
  return new Promise((resolve, reject) => {
    potrace.trace(input, options, (err: Error | null, svg: string) => {
      if (err) reject(err);
      else resolve(svg);
    });
  });
...
server.tool(
  "trace_to_svg",
  `Convert a raster image to SVG using potrace vectorization. Best for line art, diagrams, logos, and high-contrast images. The image must already exist in ${SAVE_DIR} (use save_image to import first).`,
  {
    filename: z.string().describe(`Filename of the source image in ${SAVE_DIR}`),
    threshold: z
      .number()
  ...
  async ({ filename, threshold, invert, turdsize }) => {
    try {
      ...
      if (threshold >= 0) opts.threshold = threshold;
      ...
      return {
        content: [{ type: "text" as const, text: `trace_to_svg failed: ${msg}` }],
        isError: true,
      };
    }
  }
